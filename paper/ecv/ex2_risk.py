...
The script generates data, computes the empirical risk, and saves the results in CSV files.
...
def run_one_simulation(n, phi, rho_ar1, sigma, predictor, lam, M, i, bootstrap=bootstrap):
    ...
    try:
        k_list, _, risk = compute_prediction_risk(X, Y, X_test, Y_test, predictor, lam, M, 
...
with Parallel(n_jobs=8, verbose=0, timeout=99999) as parallel:
    file_saved = path_result+'res_risk_rho_{:.02f}.csv'.format(rho_ar1)
    df_res = pd.DataFrame()
    for i_phi in tqdm(np.arange(len(phi_list)), desc = 'phi'):
        ...
        res = pd.DataFrame(np.mean(np.stack(res,axis=0), axis=0), columns=np.append(
...
#             path_result = 'result/ex2/{}/{}/{}/'.format(func, bagging,predictor)
#             file_saved = path_result+'res_risk_rho_{:.02f}.csv'.format(ar1rho)
#             _df = pd.read_csv(file_saved)
#             _df['ar1rho'] = ar1rho
#             _df.to_pickle('result/ex2/res_risk_{}_{}_{}.pkl'.format(
