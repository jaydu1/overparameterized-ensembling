...
lam_max = np.max(X.T @ y)/n
lam_min = lam_max * 1e-2
lam_list = np.round(np.unique(np.append([.003, .004, .005, .01, .05, .1], np.logspace(np.log10(lam_min), np.log10(lam_max), num=30))),4)
...
def one_run(seed, k=n//5, M=5, lam=.1):
    ...
    """
    # Note that we are solving ||y-Xb||^2/(2n) + lambda * ||b||^2
    ...
    for m, sub in enumerate(subsamples):
        ...
        clf = linear_model.Lasso(fit_intercept=False, alpha=lam)
        clf.fit(X_I, y_I)
        B[:, m] = clf.coef_
