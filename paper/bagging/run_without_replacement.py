...
def run(phi, lam, SNR):
    ...
    M_list = M_list[phis_list<=10.]
    phis_list = phis_list[phis_list<=10.]
    ...
    for M, phi_s in tzip(M_list, phis_list):
        ...
        for i in range(n_simu):
            ...
            risk_emp = comp_empirical_risk(X, Y, X_test, Y_test, phi_s, lam, 
                                           M=M, replace=False, return_allM=True)
...
for lam in lam_list:#[lam_list[int(sys.argv[1])]]:
    for phi in [0.1, 1.1]:
        for SNR in [1., 2., 3, 4.]:
#             if os.path.exists(
#                 path_result+'res_phi_{:.01f}_lam_{:.01f}_SNR_{:.01f}_without_replacement.csv'.format(phi, lam, SNR)):
#                 continue
            print(phi, lam, SNR)
            run(phi, lam, SNR)
