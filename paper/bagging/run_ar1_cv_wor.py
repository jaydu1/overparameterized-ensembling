...
def run_one_simulation(n, phi, rho_ar1, sigma, lam, M, i):
    np.random.seed(i)
    Sigma, beta0, X, Y, X_test, Y_test, rho2, sigma2 = generate_data_ar1(p, phi, rho_ar1, sigma)

    risk_emp_cv_M = cross_validation(X, Y, X_test, Y_test, lam, M, nu=0.6, replace=False)
    ...
    for lam in [lam_list[int(sys.argv[1])]]:
        for rho_ar1 in rho_ar1_list:
            ...
            for phi in tqdm(np.logspace(-1, 1, 50), desc = 'phi'):
                ...
                df_res.to_csv(path_result+'res_cv_rhoar1_{:.01f}_lam_{:.01f}_without_replacement.csv'.format(rho_ar1, lam))
